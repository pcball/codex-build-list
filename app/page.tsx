...
export default function Home() {
  ...
  const [projects,setProjects] = useState<string[]>(projectNames(seed)); const [newProject,setNewProject] = useState(""); const [addingProject,setAddingProject] = useState(false); const [editingProject,setEditingProject] = useState<string|null>(null); const [projectError,setProjectError] = useState("");
  ...
  function closeProjectForm(){setAddingProject(false);setEditingProject(null);setNewProject("");setProjectError("")}
  function beginRenameProject(project:string){setAddingProject(false);setEditingProject(project);setNewProject(project);setProjectError("")}
  function saveProject(e:FormEvent){e.preventDefault();const name=newProject.trim();if(!name){setProjectError("請輸入項目名稱。");return}if(name.length>40){setProjectError("項目名稱最多 40 個字。");return}if(projects.some(project=>project===name&&project!==editingProject)){setProjectError("這個項目已經存在。");return}if(editingProject){if(name!==editingProject){setProjects(current=>current.map(project=>project===editingProject?name:project));setTasks(current=>current.map(task=>task.category===editingProject?{...task,category:name}:task));if(projectFilter===editingProject)setProjectFilter(name)}}else setProjects(current=>[...current,name]);closeProjectForm()}
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <section className="project-manager" aria-label="項目管理"><div className="project-manager-title"><span>所屬項目</span><button aria-label="新增項目" onClick={()=>{setAddingProject(true);setEditingProject(null);setNewProject("");setProjectError("")}}><Icon name="plus" size={15}/></button></div><button className={`project-link ${projectFilter==="全部"?"selected":""}`} onClick={()=>setProjectFilter("全部")}><span>全部項目</span><b>{tasks.length}</b></button>{projects.map(project=><div className={`project-row ${projectFilter===project?"selected":""}`} key={project}><button className="project-link" onClick={()=>setProjectFilter(project)}><span>{project}</span><b>{tasks.filter(task=>task.category===project).length}</b></button><button className="project-edit" aria-label={`更改 ${project} 的名稱`} title="更改項目名稱" onClick={()=>beginRenameProject(project)}><Icon name="edit" size={13}/></button></div>)}{(addingProject||editingProject)&&<form className="project-form" onSubmit={saveProject}><input autoFocus maxLength={40} value={newProject} onChange={e=>setNewProject(e.target.value)} placeholder="輸入項目名稱" aria-label={editingProject?"新的項目名稱":"項目名稱"}/><div><button type="button" onClick={closeProjectForm}>取消</button><button type="submit">{editingProject?"儲存":"加入"}</button></div>{projectError&&<small>{projectError}</small>}</form>}<button className="add-project-button" onClick={()=>{setAddingProject(true);setEditingProject(null);setNewProject("");setProjectError("")}}><Icon name="plus" size={14}/>新增項目</button></section>
